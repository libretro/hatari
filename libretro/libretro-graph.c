...
/* Hatari always 16-bit */
#define pix_bytes 2
extern unsigned short int bmp[];
extern int retrow, retroh;
#define retro_bmp ((uint16_t*)bmp)
#define retro_buf ((uint16_t*)bmp)
#define RETRO_BMP_SIZE (1024*1024)

unsigned short int graphed[1024*1024];
      ...
      uint16_t fg, uint16_t bg)
{
   ...
   for (ypixel = 0; ypixel < charh + 1; ypixel++)
   {
      ...
      {
         b = libretro_font_array[(string[col])*charw + ypixel - 1];
